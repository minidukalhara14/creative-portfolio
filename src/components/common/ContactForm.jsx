  ...
  return (
    ...
    <div className="w-[95%] max-w-[500px] md:w-[500px] h-auto md:h-[600px] text-primary p-6 flex flex-col gap-6 items-center relative bg-secondary rounded-2xl shadow-md text-card-foreground">
      ...
      <div className="text-center w-full mt-2 md:mt-5">
        <p className="text-xl text-muted-foreground">Have a question?</p>
        <p className="text-sm text-muted-foreground mt-2">Drop me a line. I'll get back to you within 24 hours.</p>
      </div>
      ...
      <form ref={formRef} className="w-full mt-3 flex justify-center items-center flex-col gap-4" onSubmit={handleSubmit}>
        ...
        <div className="mt-1 md:mt-3 flex flex-col gap-1.5 w-full">
          ...
          <input type="email" id="email" value={formData.email} onChange={handleChange} placeholder="ishar@example.com" className="w-full px-3 py-2 border border-black/20 rounded-md bg-background text-sm focus:outline-none" required />
        </div>

        <div className="mt-1 md:mt-3 flex flex-col gap-1.5 w-full">
          <label htmlFor="message" className="text-sm font-medium">Message</label>
          <textarea id="message" rows={4} value={formData.message} onChange={handleChange} placeholder="How can i help you?" className="w-full px-3 py-2 border border-black/20 rounded-md bg-background text-sm resize-none focus:outline-none" required />
        </div>
