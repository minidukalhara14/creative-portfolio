...
export default function ReviewSlider() {
  ...
  return (
    ...
    <div className="w-full px-4 md:px-8 lg:w-[calc(100vw-70px)] bg-secondary text-white py-16 md:py-24 flex flex-col justify-center items-center border-t rounded-2xl lg:rounded-t-none lg:rounded-b-2xl border-white/5 mt-6 lg:mt-0 mx-auto">
      ...
      <div className="w-full max-w-4xl bg-black/20 mx-auto flex flex-col items-center text-center rounded-2xl p-5 md:p-8 transition-all duration-300 hover:bg-black/25 hover:shadow-2xl">
        
        
        <span className="text-primary/40 text-5xl md:text-6xl mb-4 md:mb-6 select-none">
          <GoCodeReview />
        </span>
        ...
        <div className="min-h-[140px] md:min-h-[180px] flex items-center justify-center px-2 md:px-12">
          <AnimatePresence mode="wait">
            <motion.p
              ...
              className="text-base md:text-2xl font-normal leading-7 tracking-[-0.01em] text-primary/90 max-w-[42ch] sm:max-w-[50ch]"
            >
              "{current.reviewText}"
            </motion.p>
          </AnimatePresence>
        ...
        <div className="mt-6 md:mt-8 flex flex-col items-center gap-3">
          ...
          <div>
            <h4 className="text-sm md:text-base font-medium tracking-[-0.01em] text-primary">{current.clientName}</h4>
            <p className="label text-xs md:text-sm text-primary/50 mt-0.5">{current.designation}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6 md:gap-8 mt-8 md:mt-12 select-none">
          <button 
            onClick={prevReview}
            className="w-10 h-10 md:w-12 md:h-12 rounded-full border border-white/15 flex items-center justify-center text-primary/60 hover:text-primary hover:border-white/40 hover:bg-white/[0.03] transition-all duration-300 group"
          >
            <AnimatedButton text={<GrFormPreviousLink className="text-lg md:text-xl" />} />
          </button>
          
          <span className="label text-xs md:text-sm font-normal text-primary/30">
